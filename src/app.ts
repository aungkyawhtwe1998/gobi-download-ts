...
const msToTime = (duration: number) => {
  var milliseconds = Math.floor((duration % 1000) / 100),
    seconds = Math.floor((duration / 1000) % 60).toString(),
    minutes = Math.floor((duration / (1000 * 60)) % 60).toString(),
    hours = Math.floor((duration / (1000 * 60 * 60)) % 24).toString();
​
...
const download = (url: string, path: string) =>
  new Promise((resolve, reject) => {
    http.get(url, (response) => {
      let fileType = (response.headers['content-type'])?.toString()
        .split('/')[1]
        .split(';')[0];
...
​
const getStory = async (viewKey:any) => {
  try {
...
app.get('/video',async(req:Request, res:Response, next:NextFunction)=> {
  const story = await getStory(req.query.viewKey);
  await downloadAndSaveStory(story);
  await getVideoDimensions(story);
  await downloadSaveResizeStickers(story);
  await addStickersAndSubtitlesToStory(story);
  console.log(req.query.viewKey);
  res.setHeader('Content-Type', 'video/mp4');
  res.sendFile(
    `assets/${req.query.viewKey}/storyWithStickers.mp4`,
    { root: "." }
  );
})
